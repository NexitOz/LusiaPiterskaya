...
export function UpcomingHit() {
  ...
  const toggleSound = () => {
    ...
    if (video.paused) void startVideo();
  };
  return <section className="hit-premiere" aria-labelledby="upcoming-hit-title">
    ...
    <div className="container relative z-10"><motion.div className="hit-stage" initial={reduceMotion ? false : { opacity: 0, y: 54 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <div className="hit-video-shell"><div className="hit-frame-orbit" aria-hidden="true"/><video ref={videoRef} className="hit-video" poster={upcomingHit.poster} preload="metadata" muted={muted} loop playsInline onPlay={() => { setStarted(true); setError(false); }} onError={() => setError(true)}>{upcomingHit.videoSources.map(src=><source key={src} src={src} type="video/mp4"/>)}</video><div className="hit-video-shade" aria-hidden="true"/>{!started&&<button type="button" className="hit-play" onClick={startVideo} aria-label="Смотреть тизер"><Play fill="currentColor" size={20}/><span>{error ? 'ПОВТОРИТЬ' : 'СМОТРЕТЬ ТИЗЕР'}</span>{error&&<small>Видео временно недоступно</small>}</button>}<div className="hit-corner hit-corner-top">EXCLUSIVE</div><div className="hit-corner hit-corner-bottom">01 / PREMIERE</div></div>
    </motion.div></div>
  </section>;
}
